...
import {
  isNftTransferTypeParams,
  isSwagTypeParams,
  isTransferTypeParams,
  IWalletService,
...
export class TransferUseCase extends UseCaseBase<void> {
  ...
  async handle(input: TransferInput): Promise<void> {
    const { shouldFundWallet } = input
    let optionsJSON = input.optionsJSON

    if (shouldFundWallet && (isTransferTypeParams(input) || isNftTransferTypeParams(input) || isSwagTypeParams(input)))
      await this.walletService.postFundWallet({ address: input.to })
